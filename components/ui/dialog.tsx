...
export function Dialog({ open, onOpenChange, children }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      />
      <div className="relative z-50 w-full max-w-lg scale-100 p-4">
        {children}
      </div>
    </div>
  );
...
export function DialogContent({
  ...
  return (
    <div
      className={cn(
        "relative rounded-xl border bg-background p-6 shadow-2xl duration-200 animate-in fade-in-0 zoom-in-95",
        className
      )}
    >
      {onClose && (
